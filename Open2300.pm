...
package Net::NIS::Master;

# FIXME: what?  Array of maps?  Hash, with map => master?
#############
#  TIEHASH  #  establish the relationship between a hash and the list of maps
#############
sub TIEHASH {
  my $class = shift;

  # Second argument (optional) is the NIS domain.  If unset, bail out
  # now, setting error to NODOM ("Local domain name not set").
  # if we try the yp_match, it fails with the less-than-helpful BADARGS.
  my $domain = shift || Net::NIS::yp_get_default_domain()
    or do {
	$Net::NIS::yperr = YPERR_NODOM();
	return undef;
    };

  # All OK.  Force $yperr to OK, and return a blessed object
  $Net::NIS::yperr = Net::NIS::YPERR_SUCCESS();
  bless { domain => $domain }, $class;
}


###########
#  FETCH  #  Given a key (map name), return the master
###########
sub FETCH {
    my $self = shift;			# in: our blessed self
    my $map  = shift;			# in: map name

    return Net::NIS::yp_master($self->{domain}, $map);
}


############
#  EXISTS  #  Does a map exist?
############
sub EXISTS {
    my $self = shift;
...
##############
#  FIRSTKEY  #  Fetch the list of all maps, and return the first
##############
sub FIRSTKEY {
    my $self = shift;

    my @maps = Net::NIS::yp_maplist( $self->{domain} );
...
#############
#  NEXTKEY  #  Each subsequent entry in the map list, or undef at end
#############
sub NEXTKEY {
    my $self = shift;

    return shift @{$self->{_all_maps}};
}
